...
class Slider:
    def __init__(self, window, lower_bound, upper_bound, slider_center, rail_length, rail_height, knob_length, knob_height, border_size, starting_value):
        self.window = window
        ...
        self.knob = pygame.Rect(self.slider_center[0] - self.rail_length / 2 + starting_value / (self.upper_bound - self.lower_bound) * self.rail_length, 
                                self.slider_center[1] - self.knob_height / 2, 
                                self.knob_length, 
                                self.knob_height)
        
        self.knob_bg = pygame.Rect(self.slider_center[0] - self.rail_length / 2 + starting_value / (self.upper_bound - self.lower_bound) * self.rail_length - self.border_size, 
    ...
    def getValue(self): 
        knob_x_pos = (self.knob.centerx - self.rail.x - self.knob_length / 2)
        rail_length = (self.rail.width - self.knob.width)
        bounds_difference = (self.upper_bound - self.lower_bound)
        return self.lower_bound + knob_x_pos / rail_length * bounds_difference
